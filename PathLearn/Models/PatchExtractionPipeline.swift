...
enum PatchExtractionPipeline {
    ...
    struct Config: Sendable {
        ...
        /// If set, patches with fewer than this many detected nuclei are skipped
        /// before feature extraction — keeps only patches with visible cell
        /// nuclei (excludes lumen/stroma/empty fields).
    ...
    private struct ReadyPatch: Sendable {
        ...
        let nucleusCount: Int
    }

    /// Count nuclei in a patch via classical hematoxylin deconvolution +
    /// connected components. Used to gate extraction on "has visible nuclei".
    nonisolated static func nucleusCount(of cgImage: CGImage) -> Int {
        guard let stain = StainDeconvolution.deconvolve(cgImage) else { return 0 }
        let tissue = stain.isBackground.map { !$0 }
        let result = ClassicalNucleusSegmenter().segment(stain: stain, tissueMask: tissue)
        return result.nuclei.count
    }
    ...
    nonisolated static func run(
        ...
        for (i, task) in tasks.enumerated() {
            ...
            // Count nuclei for every patch (cheap vs. the embedding) so the
            // score is stored on the bank for later filters (training, t-SNE),
            // and gate extraction on it when the "require visible nuclei" filter
            // is on — before the expensive feature extraction.
            let nuclei = nucleusCount(of: img)
            if let minN = config.minNuclei, nuclei < minN {
                skippedCount += 1
                progress(i + 1, total)
                continue
            }
