...
final class MLPatch {
    ...
    /// Number of nuclei detected in the patch (classical hematoxylin segmenter),
    /// recorded only when the "require visible nuclei" filter was active at
    /// extraction. `-1` = not measured (legacy patches, or the filter was off).
    var nucleusCount: Int = -1
    ...
    init(slidePath: String,
        ...
        self.nucleusCount = nucleusCount
        self.createdAt = Date()
    }
