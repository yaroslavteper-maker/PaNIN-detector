...
struct PredictAnnotationSheet: View {
    ...
    @State private var aggregation: PredictionPipeline.Aggregation = .perPatch
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                if classifier.isPooled {
                    Label("Pooled model: one decision for the whole annotation. The patch/stride below only controls how it's sampled; every tile is painted with that single verdict.", systemImage: "square.stack.3d.up")
                        .font(.caption2)
            ...
            if classifier.isGeometry {
                Label("Geometry model: classifies the whole annotation from its architectural descriptors (nuclear crowding, luminal complexity, etc.). The patch/stride below only controls how the single verdict is painted.", systemImage: "square.on.square.dashed")
                    .font(.caption2)
                    .foregroundStyle(.blue)
            }

            if !classifier.isPooled && !classifier.isGeometry {
                HStack {
                    Text("Decision:").frame(width: 90, alignment: .trailing)
                    Picker("", selection: $aggregation) {
                        ForEach(PredictionPipeline.Aggregation.allCases) { agg in
                            Text(agg.displayName).tag(agg)
                        }
                    }
                    .labelsHidden()
                }
                if aggregation.isPooled {
                    Text("Late fusion: classifies every tile, then combines them into ONE label for the whole annotation. Uses your full per-patch model — best when you have many patches but few annotations. “Worst focus” calls the lesion by its most atypical area.")
                        .font(.caption2)
                        .foregroundStyle(.blue)
                }
            }
            ...
            VStack(alignment: .leading, spacing: 2) {
                HStack(spacing: 6) {
                    Toggle("Require at least", isOn: $requireNuclei)
                        .toggleStyle(.checkbox)
                        .font(.caption)
                    Stepper(value: $minNuclei, in: 1...200) {
                        Text("\(minNuclei) nuclei per patch")
                            .font(.caption.monospacedDigit())
                    }
                    .disabled(!requireNuclei)
                    .fixedSize()
                }
                Text("Each sampled patch is checked for nuclei (hematoxylin deconvolution) before classifying; patches with too few are skipped — no verdict, no heatmap cell. Excludes lumen, stroma, and empty fields from the result.")
                    .font(.caption2)
                    .foregroundStyle(.tertiary)
            }
            ...
            HStack {
                ...
                Button("Run") {
                    ...
                    onRun(
                        PredictionPipeline.MultiPassConfig(
                            perClass: perClass,
