...
struct ClassEditorSheet: View {
    ...
    init(classification: Classification,
        ...
        _isNull = State(initialValue: classification.isNull)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Toggle("Exclude / null class", isOn: $isNull)
                    .toggleStyle(.checkbox)
                Text("Mark this class as a null reference (e.g. PaNIN lumens). Its patches are never learned as a real class, and candidate patches that resemble them are excluded from training and prediction.")
                    .font(.caption)
