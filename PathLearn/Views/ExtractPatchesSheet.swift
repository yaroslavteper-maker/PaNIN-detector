            ...
            VStack(alignment: .leading, spacing: 2) {
                HStack(spacing: 6) {
                    ...
                    Stepper(value: $minNuclei, in: 1...200) {
                        Text("\(minNuclei) nuclei")
                            .font(.caption.monospacedDigit())
                    }
                    .disabled(!requireNuclei)
                    .fixedSize()
                }
                Text("Detects nuclei in each patch (hematoxylin deconvolution) and skips patches with too few — keeps only patches with visible cell nuclei, excluding lumen, stroma, and empty fields.")
                    .font(.caption2)
                    .foregroundStyle(.tertiary)
            }
